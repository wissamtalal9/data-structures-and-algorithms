...
const toTitleCase = (arr) => {
  // Solution code here...
  let Title = arr.map(element => 
    {
    let upperValuenew = element[0].toUpperCase();
    for (let i=1; i < element.length ; i++)
     {
      upperValuenew += element[i];
    }
    return upperValuenew;
  });
  return Title;
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
   const checki = (row1, col1, row2, col2, row3, col3) => {
    return board[row1][col1] !== '' &&
      board[row1][col1] === board[row2][col2] &&
      board[row2][col2] === board[row3][col3];
  };
  if (checki(0, 0, 0, 1, 0, 2)) return true;
  if (checki(1, 0, 1, 1, 1, 2)) return true;
  if (checki(2, 0, 2, 1, 2, 2)) return true;
  if (checki(0, 0, 1, 0, 2, 0)) return true;
  if (checki(0, 1, 1, 1, 2, 1)) return true;
  if (checki(0, 2, 1, 2, 2, 2)) return true;
  if (checki(0, 0, 1, 1, 2, 2)) return true;
  if (checki(0, 2, 1, 1, 2, 0)) return true;
  return false;
