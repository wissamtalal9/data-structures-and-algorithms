...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  return (starWarsArr.sort((first_str, second_str) =>
   (second_str.height - first_str.height)))
...
const listFoods = (recipe) => {
  ...
  //   let result= recipe.ingredients[i].split(' ').splice(2).join(' ');
  //   result.push(result);
  // }

  result = recipe.ingredients.map(item=>
    item.split(' ').slice(2,item.length).join(" "))
