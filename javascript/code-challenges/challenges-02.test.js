...
------------------------------------------------------------------------------------------------ */
// (hint: look up Math.pow())
const raisedToTheThird = (arr) => {
  arr.foreach(item=> arr.push(Math.pow(item, 3));
  return arr;
  // Solution code here...
};
...
// return NewArray;
  arr.map(item=>{
    item % 2 == 0 ? NewArray.push(arr.name + ' Even');
    item % 2 != 0 ? NewArray.push(arr.name +' Odd');
  });
return NewArray
};
...
const extractStats = (arr) => {
  // Solution code here...
  snorlaxAbilities.map(Element=>{
    arr.push( Element.stat.url, Element.stat.name)
  })
  return arr;
};
