...
const count = (target, input) => {
  ...
  return Arrays.filter((item) => item === target).length;
};
...
const totalSum = (input) => {
  // Solution code here...
  const sumvariable = input.reduce((Finally, summation) => {
    return Finally += summation.reduce((responsevie, number2) => {
      return responsevie += number2;
    }, 0);
  }, 0);
  return sumvariable;
};
  ...
  input.map((arrays) => {
    ...
    arrays.map((item) => {
      if (typeof item === 'number' && item % 5 === 0) {
        arrayes_Two.push(Math.pow(2, item));
      }
    });
    Arrayes.push(arrayes_Two);
  });
  return Arrayes;
};
...
let findMaleAndFemale = (data) => {
  ...
  const FilterResult = Filtering.map((item) => item.name);
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((responsive, items) => {
    return responsive = Number(items.height) < Number(responsive.height) ? items : responsive;
  }).name;
};
